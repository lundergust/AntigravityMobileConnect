...
(function() {
    ...
    if (containers.length === 0) return JSON.stringify({ error: "No containers" });
    ...
    if (!foundProps) return JSON.stringify({ error: "Props not found" });

    const steps = foundProps.trajectory.steps;
    
    // Check specific indices where we expect data
    const relevantIndices = [0, 46];
    ...
    const results = steps
        ...
        .map(x => {
            ...
            // Extract raw data without complex objects
            return {
                index: x.index,
                keys: Object.keys(step),
                name: step.name,
                argumentsJson: step.argumentsJson, // Raw JSON string
                userResponse: step.userResponse, // Raw user response
                items: step.items, // Raw items
                type: step.type
            };
        });

    return JSON.stringify({ selectedSteps: results }, null, 2);
})()
"""
...
async def main():
    ...
    result = await conn.evaluate(DEBUG_JS) # Should return a string now
    # print(result) # It's already JSON string
    try:
